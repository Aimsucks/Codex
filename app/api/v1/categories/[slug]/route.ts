...
export async function DELETE(request: NextRequest, {params}: { params: { slug: string } }) {
    ...
    const category: Category & { subcategories: Category[] } | null = await prisma.category.findUnique({
        where: {id},
    ...
    if (category.subcategories.length) return NextResponse.json({message: "Cannot delete category that has presets"}, {status: 400})
